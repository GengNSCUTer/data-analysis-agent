...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Code\nSELECT 1 AS gmv", "SELECT 1 AS gmv"),
        ("Selection\nWITH totals AS (SELECT 1 AS gmv) SELECT gmv FROM totals", "WITH totals AS (SELECT 1 AS gmv) SELECT gmv FROM totals"),
        ("Solution WITH totals AS (SELECT 1 AS gmv) SELECT gmv FROM totals", "WITH totals AS (SELECT 1 AS gmv) SELECT gmv FROM totals"),
        ("Query SELECT 1 AS gmv", "SELECT 1 AS gmv"),
        ("Here is SQL: SELECT 1 AS gmv", "Here is SQL: SELECT 1 AS gmv"),
        ("Proposal\nWITH totals AS (SELECT 1 AS gmv) SELECT gmv FROM totals", "WITH totals AS (SELECT 1 AS gmv) SELECT gmv FROM totals"),
        ("Answer\nSELECT 1 AS gmv", "SELECT 1 AS gmv"),
        ("SQL Query\nSELECT 1 AS gmv", "SELECT 1 AS gmv"),
        ("query\nSELECT 1 AS gmv", "SELECT 1 AS gmv"),
        ("Query details\nSELECT 1 AS gmv", "Query details\nSELECT 1 AS gmv"),
        ("Proposal\nnot SQL prose", "Proposal\nnot SQL prose"),
        ("Proposal; DROP TABLE fact_orders\nSELECT 1 AS gmv", "Proposal; DROP TABLE fact_orders\nSELECT 1 AS gmv"),
