...
def test_candidate_builder_overfetch_counts_only_valid_source_rows(
    monkeypatch, tmp_path: Path
) -> None:
    train_path = tmp_path / "train_spider.json"
    tables_path = tmp_path / "tables.json"
    ...
    train_path.write_text(
        json.dumps(
            [
                ...
                {"db_id": "shop", "query": "SELECT name FROM products"},
            ]
        ),
        encoding="utf-8",
    )
    ...
    assert candidates[0]["sample_id"] == "spider_train:00000"
