...
> 已重组：从现在起先阅读 [后训练研究总览](../README.md)，概念学习使用
> [学习指南](../learning/fundamentals.md)，实验事实使用
> [实验台账](../experiments/log.md)。本文保留为原始路线草案，避免把历史材料与当前状态混在一起。
...
每次实验都记录：问题、原理假设、配置、GPU 映射、数据版本、训练/验证结果、失败排查、与冻结基线的对比、面试表达。2026-08-25 已完成 128 条 train-only 候选的 tokenizer/forward 及 8-step QLoRA SFT 工程 smoke；下一轮要先建立同基座、同提示、同解码和同 Spider dev/Test Suite 协议下的前后对比，再讨论模型质量，而不是用训练 loss 下结论。
