...
扩展决策（新增指标、family 配额和中文 surface contract）见 [`data/olist-domain-sft-expansion-decision-v1.md`](data/olist-domain-sft-expansion-decision-v1.md)。其设计先落地为 v3.0，随后已由 [`data/olist-v3-balanced-release-v1-1.md`](data/olist-v3-balanced-release-v1-1.md) 与 [`data/olist-v3-1-balance-and-surface-repair-contract.md`](data/olist-v3-1-balance-and-surface-repair-contract.md) 修正为当前 4,500 行 v3.1 release；默认运行时仍固定在 v2，v3.1 尚未启动训练。
...
| R5 领域数据与受控接入 | 构建运行时 Prompt 对齐的 Olist 领域资产，并在通过质量门后再讨论候选接入 | v2 对照已完成；当前 v3.1 平衡纯中文数据已物化，尚未训练或接入 | Release v2 的既有对照仍只证明共享协议拟合，不能外推。当前 v3.1 release 在隔离 `olist-demo-v3` workspace 重建为 train/validation/final-test `3,000/750/750`：4,500/4,500 canonical Gold SQL 通过 Policy、reader role 和结果合同；每实例八问法的 36,000 条纯中文 runtime overlay 已重建，主问法按 split×bucket 分层精确配额选择；长度上限 3072 无排除。48 条 advisory 复核为 45 条 pass 与 3 条 provider timeout，timeout 仅作为非阻塞审计警告保留；family、QuerySpec、canonical SQL hash 跨 split 为零交集。它仍是训练前数据资产，未证明 Adapter 提升或生产接入资格，详见 [`data/olist-v3-balanced-release-v1-1.md`](data/olist-v3-balanced-release-v1-1.md)。 |
