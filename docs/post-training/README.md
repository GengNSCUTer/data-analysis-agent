...
| R7 Qwen3.5 Instruct 迁移准备 | 在不改 Olist split、不读取 TheLook 的前提下，验证 Qwen3.5-4B bf16 LoRA 训练闭环 | 进行中：Trainer smoke 通过；正式两 epoch 受共享 GPU 资源门阻塞 | 专用 Trainer 以官方 Qwen3.5 chat template 监督 SQL + EOT，严格将 LoRA 限于真实 `model.language_model.layers.*` 的 128 个 projection（attention 各 8、MLP 各 32），不按全模型 suffix 盲匹配。1-step smoke 以最短的 train/validation 冻结行完成 train/eval/checkpoint/adapter final/fresh reload，loss 均有限，峰值 `12,204.7/13,384.0 MiB` allocated/reserved；这不代表最长 `2862/2874` token 样本可训练。首次 2,498-token validation 已需要额外 2.31 GiB 而 OOM，故正式 launcher 在物理 GPU 2 空闲小于 17,408 MiB 时 fail closed；当前不抢占他人任务。TheLook 未读取。详见 [`experiments/qwen35-4b-olist-instruct-sft-smoke-v1.md`](experiments/qwen35-4b-olist-instruct-sft-smoke-v1.md)。 |
