...
| R6 跨 schema 迁移评测 | 保持 Olist LoRA 冻结，在第二个未见电商 schema 上作 Base/Adapter 零样本对照 | Workspace 与首批 Catalog 完成，尚未生成题目或评测 | TheLook Kaggle mirror 已冻结、导入独立 PostgreSQL `thelook_analytics`，通过源完整性/最小权限审计，并冻结 `thelook-catalog-v1` 六项指标与八条 Join。它不进入 Olist 训练、验证、Prompt 示例或调参；下一步才是审阅 QuerySpec/Gold SQL 可表达范围。详见 [`data/thelook-cross-schema-workspace-v1.md`](data/thelook-cross-schema-workspace-v1.md)。 |
