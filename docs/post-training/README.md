...
| R6 跨 schema 迁移评测 | 保持 Olist LoRA 冻结，在第二个未见电商 schema 上作 Base/Adapter 零样本对照 | Workspace、Catalog 与 QuerySpec 合同完成，尚未生成 Gold SQL/题目 | TheLook Kaggle mirror 已冻结、导入独立 PostgreSQL `thelook_analytics`，通过源完整性/最小权限审计，冻结 `thelook-catalog-v1` 六项指标、八条 Join，并完成 `thelook-query-spec-v1` 结构校验。它不进入 Olist 训练、验证、Prompt 示例或调参；下一步才是 deterministic Gold SQL renderer。详见 [`data/thelook-cross-schema-workspace-v1.md`](data/thelook-cross-schema-workspace-v1.md) 与 [`data/thelook-queryspec-contract-v1.md`](data/thelook-queryspec-contract-v1.md)。 |
