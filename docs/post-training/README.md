...
| R6 跨 schema 迁移评测 | 保持 Olist LoRA 冻结，在第二个未见电商 schema 上作 Base/Adapter 零样本对照 | Workspace、Catalog、QuerySpec 与 renderer 完成，尚未生成题目/评测 | TheLook Kaggle mirror 已冻结、导入独立 PostgreSQL `thelook_analytics`，通过源完整性/最小权限审计，冻结 `thelook-catalog-v1` 六项指标、八条 Join，完成 `thelook-query-spec-v1` 结构校验和 `thelook-postgres-gold-renderer-v1`；五个代表性 artifact 已通过 Policy 并用 reader 执行。下一步才是基于 Gold 构造约 200 条测试题。详见 [`data/thelook-cross-schema-workspace-v1.md`](data/thelook-cross-schema-workspace-v1.md)、[`data/thelook-queryspec-contract-v1.md`](data/thelook-queryspec-contract-v1.md) 与 [`data/thelook-renderer-contract-v1.md`](data/thelook-renderer-contract-v1.md)。 |
