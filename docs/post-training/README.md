...
| R7 Qwen3.5 Instruct 迁移 | 用同一冻结 Olist SFT 合同对比 2B 与 4B bf16 LoRA，并只在 protected TheLook 上做后置跨 schema 评测 | 进行中：2B TheLook v2 对照已完成；4B 两 epoch 正在运行 | 2B 的 1,200 steps / 2 epoch 已完成，最后 train/validation loss=`6.2284e-06/6.5842e-06`，fresh reload 有限；该极低 in-domain loss仅证明共享 Catalog/QuerySpec/renderer 的协议拟合。TheLook v2 的 matching Base/Adapter `600/600` generation、pair marker、统一 Policy/reader/ResultContract 与生成后 Gold 对照均完成：Policy accepted `172 -> 388`、PostgreSQL executed `78 -> 206`、ResultContract valid `58 -> 196`、ordered-or-bag denotation match `23 -> 118`。同时仍有 `33` 条 valid -> non-valid 回退，且 Adapter 的 `71` 条合同有效 case 存在值不匹配，不能写为开放式泛化、无回退质量门或生产接入资格。4B 同合同两 epoch 已在 UUID guard 下运行；两个模型均不改变生产默认路径。 |
| R8 Schema-aware Program SFT | 从当前 Qwen2.5-Coder 1.5B Base 训练新 Adapter，以训练期结构化程序监督补足 schema linking / grain / result-contract 能力 | Phase 1：结构计划 registry/derive/validate 已完成；未物化数据或训练 | Task B `SchemaLinkPlan` 已从已验证 Olist QuerySpec、Catalog 和独立静态 registry 确定性派生表列绑定、真实 Catalog Join ID、粒度、时间 owner、filter/dedup、分组和 alias；Task A 保持生产 Prompt -> SQL 不变，部署仍一次生成 SQL。10 指标/形态/AOV/tamper 回归与既有 QuerySpec 共 `66 passed`。TheLook v2 永不反向进入训练或模型选择。详见 [`data/schema-aware-program-sft-contract-v1.md`](data/schema-aware-program-sft-contract-v1.md)。 |
