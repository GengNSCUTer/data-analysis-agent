...
| R5 领域数据与受控接入 | 构建运行时 Prompt 对齐的 Olist 领域资产，并在通过质量门后再讨论候选接入 | 十指标 Catalog 已冻结，尚未物化 | Spider 与 CSpider 两个 Adapter 的 12 条 Olist 业务迁移子门均未通过；`olist-catalog-v2` 已冻结 10 项指标及其分母/归属边界。旧 v1 coverage 只记录 4 指标历史范围，不能直接物化。下一项仅修订 coverage matrix；归因、Top-N、自由筛选和未建模排序继续排除，安全、权限、结果与图表合同仍在服务器端执行。 |
...
15. [Olist 领域 Candidate SQL SFT 覆盖矩阵 v1](data/olist-domain-sft-coverage-matrix-v1.md)：4 指标历史快照，必须在十指标合同基础上修订后才能物化。
