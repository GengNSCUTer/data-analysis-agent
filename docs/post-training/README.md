...
| R6 跨 schema 迁移评测 | 保持 Olist LoRA 冻结，在第二个未见电商 schema 上作 Base/Adapter 零样本对照 | v1 已完成；v2 场景/指标候选已完成画像，尚未冻结或生成 | v1 的 206 条及历史 matching 结果保持不变。为避免仅复制日期扩大样本，v2 候选把范围扩至订单购物篮、履约阶段、库存/配送中心、行为/会话和用户注册五类事实域；所有候选仍待独立 Catalog、QuerySpec、renderer 和小批准入。详见 [`data/thelook-v2-metric-scenario-candidates.md`](data/thelook-v2-metric-scenario-candidates.md)。 |
