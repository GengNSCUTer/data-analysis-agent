...
| R6 跨 schema 迁移评测 | 保持 Olist LoRA 冻结，在第二个未见电商 schema 上作 Base/Adapter 零样本对照 | 基础设施与 206 条 final test 已冻结，尚未启动模型生成 | TheLook Kaggle mirror、独立 PostgreSQL workspace、最小权限、Catalog、QuerySpec 和 renderer 已完成；206 条中文 final-test 全量通过 `QuerySpec -> renderer -> Policy -> reader -> ResultValidator`，无重复问题/family/QuerySpec/Gold SQL hash，最大结果行数 194。它不进入 Olist 训练、验证、Prompt 示例或调参；下一步单独冻结 matching Base/Adapter 生成合同。详见 [`data/thelook-cross-schema-final-test-v1.md`](data/thelook-cross-schema-final-test-v1.md)。 |
