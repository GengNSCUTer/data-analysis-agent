...
| R7 Qwen3.5 Instruct 迁移 | 用同一冻结 Olist SFT 合同对比 2B 与 4B bf16 LoRA，并只在 protected TheLook 上做后置跨 schema 评测 | 进行中：2B 对照已完成；4B 训练完成，Adapter-only TheLook generation 进行中 | 2B 的 1,200 steps / 2 epoch 已完成，最后 train/validation loss=`6.2284e-06/6.5842e-06`，fresh reload 有限；该极低 in-domain loss仅证明共享 Catalog/QuerySpec/renderer 的协议拟合。TheLook v2 的 matching Base/Adapter `600/600` generation、pair marker、统一 Policy/reader/ResultContract 与生成后 Gold 对照均完成：Policy accepted `172 -> 388`、PostgreSQL executed `78 -> 206`、ResultContract valid `58 -> 196`、ordered-or-bag denotation match `23 -> 118`。同时仍有 `33` 条 valid -> non-valid 回退，且 Adapter 的 `71` 条合同有效 case 存在值不匹配，不能写为开放式泛化、无回退质量门或生产接入资格。4B 的 Olist bf16 LoRA 已完成两 epoch，当前只读 TheLook 安全投影生成 Adapter completion；在 600/600 completion、case/hash/Prompt/model/decode 配对核验通过前，不读取 Gold 或数据库，两个模型均不改变生产默认路径。 |
| R8 Schema-aware Program SFT | 从当前 Qwen2.5-Coder 1.5B Base 训练新 Adapter，以训练期结构化程序监督补足 schema linking / grain / result-contract 能力 | Phase 1：结构计划 registry/derive/validate、外部 Task A/B 物化与独立回读 audit 已完成；未改 Trainer、未训练 | 已从 Olist Release v2 train/validation 物化 `2,400/600` 个 query instance、`3,000` 个稳定 pair 和 `6,000` 个事件；Task A Prompt/SQL 源字节保持，Task B 由 QuerySpec/Catalog 确定性派生，长度审计上限 3,072 无排除。独立 audit 从可信 source 重新派生全部事件并确认 source hash、family/QuerySpec 隔离、Task A byte identity、Task B plan、pair/order 与零排除；相关回归 `82 passed`。TheLook v2 永不反向进入训练或模型选择。详见 [`data/schema-aware-program-sft-contract-v1.md`](data/schema-aware-program-sft-contract-v1.md)。 |
