...
| R5 领域数据与受控接入 | 构建运行时 Prompt 对齐的 Olist 领域资产，并在通过质量门后再讨论候选接入 | Release v2 in-domain 对照完成，尚未接入运行时 | Release v2 已冻结 train/validation/final-test `2,400/600/600`，每个 query instance 从五个中文受控表面问法中稳定选择一个主问法；family/QuerySpec 跨切分隔离。相同 600 条 final test 的 matching 评测中，Base 的 Policy/PostgreSQL/ResultContract 为 `225/60/20`，Adapter 为 `600/600/600`；生成完成后独立 Gold denotation 审计为 Base `1/20` eligible match、Adapter `600/600` ordered match。Adapter 的候选 SQL 与 Gold 逐字一致，故证据说明它已学会当前 Olist `Catalog + QuerySpec + ResultContract -> canonical SQL` 协议；同一 workspace、指标与 renderer 仍为共享条件，不能外推为开放式 SQL 准确率、跨 schema 泛化或生产接入资格。详细实验记录见 [`experiments/log.md`](experiments/log.md)。 |
