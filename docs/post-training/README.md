...
| R7 Qwen3.5 Instruct 迁移 | 用同一冻结 Olist SFT 合同对比 2B 与 4B bf16 LoRA，并只在 protected TheLook 上做后置跨 schema 评测 | 进行中：2B 两 epoch 已完成；4B 两 epoch 已启动；2B TheLook Adapter 入口已准备 | 2B 的 1,200 steps / 2 epoch 已完成，最后 train/validation loss=`6.2284e-06/6.5842e-06`，fresh reload 有限；该极低 in-domain loss 仅证明共享 Catalog/QuerySpec/renderer 的协议拟合。此前 Qwen3.5-2B Base 已在同一 protected TheLook v2 `600/600` 生成完毕，现经 case/manifest/Prompt hash、revision、官方非思考 chat template、bf16 与 greedy decode 的只读复核后可复用；Adapter 仍未启动，Gold/数据库仍未读取。Qwen3.5-4B 的同合同两 epoch 已在 UUID guard 下启动。两个模型均不改变生产默认路径。 |
