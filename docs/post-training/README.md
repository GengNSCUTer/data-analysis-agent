...
| R7 Qwen3.5 Instruct 迁移准备 | 在不改 Olist split、不读取 TheLook 的前提下，冻结 Qwen3.5-4B LoRA 的训练输入合同 | 进行中：模板与全量 token 准入已通过；Trainer smoke 尚未启动 | 既有 Olist Release v2 的 2,400 train / 600 validation 保持原始 `rendered_prompt + canonical SQL` 内容。Qwen3.5 官方 chat template 仅将运行时 Prompt 包进 user turn，assistant 仅监督 SQL 与 EOT，`enable_thinking=False`，模板前缀及空 think 标记全部 mask。全量 CPU token 审计为 train `1116--2862`、validation `1086--2874`，均低于 3072，零截断；TheLook 未读取。下一步只审阅 Qwen3.5 专用 Trainer、LoRA target modules、显存 smoke 与 matching Base/Adapter 合同，不启动正式训练。详见 [`data/qwen35-olist-sft-template-contract-v1.md`](data/qwen35-olist-sft-template-contract-v1.md)。 |
