...
| R6 跨 schema 迁移评测 | 保持 Olist LoRA 冻结，在第二个未见电商 schema 上作 Base/Adapter 零样本对照 | v1 已完成；v2 600 条 final test 与 matching 合同已冻结，尚未启动模型 | v1 的 206 条及历史结果保持不变。v2 独立冻结了 `thelook-catalog-v2`、QuerySpec、renderer 和 600 条 protected case，覆盖订单购物篮、履约阶段、库存/配送中心、行为/会话与用户注册；600/600 通过 `SqlPolicy -> daa_thelook_reader -> ResultValidator`，最大返回 194 行。matching 固定同一 Qwen bf16 Base、Olist Release v2 Adapter、真实 Prompt bundle SHA、greedy decode 和 generation → pair marker → Gold 三阶段隔离；TheLook 的问题、Gold 和结果仍不进入 Olist 训练/Prompt。详见 [`data/thelook-v2-matching-evaluation-contract.md`](data/thelook-v2-matching-evaluation-contract.md)。 |
