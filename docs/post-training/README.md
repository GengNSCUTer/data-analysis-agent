...
| R8 Schema-aware Program SFT | 从当前 Qwen2.5-Coder 1.5B Base 训练新 Adapter，以训练期结构化程序监督补足 schema linking / grain / result-contract 能力 | 完整训练完成；尚未进行 matching 生成评测 | Olist Release v2 的 train/validation 已物化 `2,400/600` 个 query instance、`3,000` 个稳定 pair 和 `6,000` 个 event；Task A Prompt/SQL 源字节保持，Task B 由 QuerySpec/Catalog 确定性派生。独立 audit、覆盖 review、Trainer 回归与最长 3,046-token smoke 均已通过。bf16 LoRA 完成 2 epoch / 2,400 optimizer step：最终 train / aggregate validation / SQL validation / SchemaLinkPlan validation loss 为 `0.114698 / 0.100952 / 0.00001549 / 0.201889`，最终 Adapter 已 fresh reload。aggregate validation 最低记录在未保存的 step 2100，当前交付是 final step 2400 Adapter，不把 loss 写成泛化或业务准确率。下一步在严格 Gold 后置隔离下做 Olist final test 与 protected TheLook v2 的 Base / SQL-only Adapter / Schema-aware Adapter matching 对照；TheLook v2 永不反向进入训练或模型选择。详见 [`data/schema-aware-program-sft-contract-v1.md`](data/schema-aware-program-sft-contract-v1.md)。 |
