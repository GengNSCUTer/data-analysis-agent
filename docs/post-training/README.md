...
| R8 Schema-aware Program SFT | 从当前 Qwen2.5-Coder 1.5B Base 训练新 Adapter，以训练期结构化程序监督补足 schema linking / grain / result-contract 能力 | Phase 1 数据准备完成；TASK-007 Trainer 入口已实现并通过逻辑回归；尚未 GPU smoke/完整训练 | Olist Release v2 的 train/validation 已物化 `2,400/600` 个 query instance、`3,000` 个稳定 pair 和 `6,000` 个 event；Task A Prompt/SQL 源字节保持，Task B 由 QuerySpec/Catalog 确定性派生。独立 audit 重新派生全部 event 并确认 source hash、family/QuerySpec 隔离、Task A byte identity、Task B plan、pair/order 与零排除；随后有界 review 确认十指标、四种结果形态、十条 Join program 全覆盖，Task B 最大 3,046 token、无越界。Trainer 在模型加载前绑定 audit/review、物化目录、event hash、Base model manifest，并分别评估两类 validation loss；相关逻辑回归 `4 passed`，既有全套回归 `87 passed`。TheLook v2 永不反向进入训练或模型选择。详见 [`data/schema-aware-program-sft-contract-v1.md`](data/schema-aware-program-sft-contract-v1.md)。 |
