...
同日启动 Schema-aware Program SFT 的第一个不使用 GPU 的实现单元：新增 `SchemaLinkPlan` 的独立静态 registry、canonical `slp_` ID、QuerySpec-derived plan 和 fail-closed validator。它从已验证的 Olist QuerySpec/Catalog/指标注册表派生 relation/alias、真实 Catalog Join ID、grain、time owner、filter/dedup、group key、CTE merge 和 result alias；不解析 renderer SQL、不调用 LLM 或数据库。实施中发现草案 Join 名称与 Olist Catalog 的实际 join ID 不一致，已在任何训练数据物化前更正为 `orders_items`、`orders_customers`、`orders_reviews`、`items_products`，并加入 Catalog 一致性守卫。十指标、四类形态、AOV、中间 grain、mapping、tamper 与跨 QuerySpec 回归连同既有 QuerySpec 共 `68 passed`。未读取 TheLook、未物化 Task A/B、未训练或修改运行时。

同日完成 Phase 1 的外部 Task A/B materialization。输入严格限定为 Olist Release v2 已通过的 train/validation JSONL、split audit、admission-v2 证据和本地 1.5B Base tokenizer；TheLook、in-domain test、数据库、LLM 与 GPU 均未读取/使用。对 `2,400 + 600 = 3,000` 个 query instance，Task A 保持源 runtime Prompt、Gold SQL 和 `training_text` 边界，Task B 从同一 QuerySpec 确定性派生 canonical `SchemaLinkPlan` JSON；每个 instance 形成一个稳定 pair、两个独立事件，未把事件计作额外语义查询。首次追加完整 runtime Prompt 会使 153 个 Task B 超过 3,072，因此改为只保留 Catalog、Query Plan、Question 的紧凑结构上下文；最终 Task B 最大 `3,046`，Task A 最大 `2,921`，`6,000/6,000` 事件成对可训练、无长度排除，审计状态 `pass`。外部物化目录为 `/disk2/gengnan/data-analysis-agent-data/experiments/olist-schema-aware-program-sft-v1-materialization-20260911c/`；下一步才实现 Trainer 入口。

随后新增独立只读 audit，而不是信任 materializer 的自描述。它只读取同一 Olist Release v2 的 source split audit、train/validation、admission-v2 和 tokenizer，重新派生全部 Task A/B event 并同外部物化逐项比较；不得读取 TheLook / in-domain test、执行 SQL、调用模型或使用 GPU。实现过程中发现空的 `exclusions/length.jsonl` 是“0 条排除”的合法状态，初版错误复用了“不允许空 JSONL”的读取边界而拒绝该合法状态；已收紧为“训练事件必须非空、exclusion manifest 可为空”并补回归。真实 audit 最终通过：`3,000` pair、Task A/B 各 `2,400/600`、交错 event `4,800/1,200`、0 exclusion，且 source hash、family/QuerySpec isolation、Task A byte identity、Task B plan、pairing 与顺序均为 true。脱敏报告为 `/disk2/gengnan/data-analysis-agent-data/experiments/olist-schema-aware-program-sft-v1-audit-20260911/audit-report.json`；相关 Python 回归 `82 passed`。下一步先完成 Olist-only 的有界分层 review，之后才改 Trainer。

同日，`Qwen/Qwen3.5-4B@851bf6e806efd8d0a36b00ddf55e13ccb7b8cd0a` 的 Olist bf16 LoRA 两 epoch 已完成，最终 Adapter 已冻结在仓库外。按照 TheLook v2 matching 合同，Adapter-only generation 已启动：只读取安全 projection、官方 Qwen3.5 non-thinking chat template、固定 server Prompt 和 greedy decode，不读 Gold SQL、数据库结果或 Base completion。只有 `600/600` Adapter completion 写完并与已冻结 Base 的 case/manifest/prompt/model/decode/raw hash 完成无 Gold 配对核验后，才会解锁 `SqlPolicy -> reader -> ResultContract -> Gold denotation` 后评测；当前不产生跨 schema 质量结论。

Adapter 的 `600/600` completion 以 exit code 0 完成，并由独立 4B verifier 在读取 Gold 前核验 Base/Adapter 的 case 顺序、case/manifest hash、服务器 Prompt bundle、官方 template/non-thinking 状态、模型 revision、bf16、greedy decode、download manifest 和 raw completion hash。matching marker 通过后，统一运行 `unwrap_sql_completion -> SqlPolicy -> daa_thelook_reader -> ResultContract/ResultValidator`，仅对 ResultContract valid 候选读取 Gold SQL 并比较结果。Base -> Adapter 的 Policy accepted / PostgreSQL executed / ResultContract valid 为 `234/155/131 -> 518/389/389`；ordered-or-bag denotation 为 `31/600 -> 340/600`（Adapter 为 337 ordered、3 bag），并有 48 条 denotation mismatch、1 条 row-count mismatch、211 条未通过合同和 12 条 Base-valid -> Adapter-non-valid 回退。该结果是冻结、未见电商 schema 上的正向迁移证据，但仍不能写成开放式 Text-to-SQL 正确率、无回退泛化或生产接入。原始 completion、SQL、Gold 与结果行均留在仓库外；脱敏汇总在 `qwen35-4b-thelook-v2-adapter-v1-20260911/evaluation/evaluation-report.json`。

同日完成 Schema-aware Program SFT Phase 1 的最后一项 Olist-only 有界 review。它先要求独立 materialization audit 为 pass，再仅从外部 Task A/B record 的结构字段与 plan target 统计指标、形态、时间、Join 和 token 长度；报告不写问题、Prompt、SQL、完整 plan JSON 或结果行。train/validation 的 2,400/600 instance 分别覆盖当前十指标、scalar/state/category/time-series 和全部十条 Join program；Task B 最大长度为 3,046/3,025、无越界。报告为 `/disk2/gengnan/data-analysis-agent-data/experiments/olist-schema-aware-program-sft-v1-review-20260911/review-report.json`。至此 Phase 1 只完成可审计数据准备；下一步才设计 1.5B Base 的 pair-aware Trainer，不启动训练。
