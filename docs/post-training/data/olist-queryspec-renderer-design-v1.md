...
本轮已完成 `QuerySpec` immutable model、验证器、只读指标表达式注册表、renderer 和确定性单元测试。
下一轮只设计受控 QuerySpec 批量物化器；不得在该设计阶段生成正式 QuerySpec、执行数据库、物化 Prompt/Gold、
进行 token 审计或启动 GPU 任务。
...
- 实现在 `src/data_analysis_agent/olist_queryspec.py`：冻结 dataclass 以 canonical JSON 生成 `query_spec_id`；验证器只接受当前 workspace 快照、十项已登记指标和 coverage v2 的静态形状。即使调用方显式传入 Catalog，其 Catalog/dataset/metric/policy 快照也必须与 QuerySpec 一致。版本漂移、指标/结果列篡改、归因、敏感卖家维度、支付归因、混合时间字段及未知字段均 fail closed。
...
- `tests/test_olist_queryspec.py` 覆盖十项标量公式/过滤、四类输出形状、AOV 粒度、多指标 CTE、半开时间范围、稳定 hash、映射往返、显式 Catalog 快照漂移及冻结拒绝码；`SqlPolicy` 兼容性作为测试断言而非 renderer 副作用。QuerySpec/renderer 专项 `46 passed`，与 Catalog/Router/QueryPlan/ResultValidator/SqlPolicy/Trusted SQL Tool 的相关回归 `162 passed`。
