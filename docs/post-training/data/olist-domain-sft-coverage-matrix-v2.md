...
**状态：** 覆盖范围已冻结；未设计 `QuerySpec`、未实现 renderer、未物化训练行、未加载 tokenizer、未启动 GPU。
...
| --- | --- | --- | --- |
| 商品行 | `gmv`、`item_count`、`freight_amount` | `order_item`；`fact_orders.order_purchase_timestamp` | 日期、客户州，商品品类、卖家。客户城市关联粒度安全但受高基数输出合同限制，暂不物化；支付方式仍需要归属/分摊。 |
...
| `JP08_seller_item` | 商品行指标按卖家 | `fact_orders -> fact_order_items -> dim_sellers` | 卖家天然属于商品行。 |
...
| --- | --- | --- | --- | --- | --- | --- |
| `gmv` | 纳入 `JP01` | 纳入 `JP04` | 暂缓：高基数输出/稳定顺序未入合同。 | 纳入 `JP07` | 纳入 `JP08` | 需澄清：支付归属/分摊未冻结。 |
| `item_count` | 纳入 `JP01` | 纳入 `JP04` | 暂缓：高基数输出/稳定顺序未入合同。 | 纳入 `JP07` | 纳入 `JP08` | 需澄清：商品行支付归属/分摊未冻结。 |
| `freight_amount` | 纳入 `JP01` | 纳入 `JP04` | 暂缓：高基数输出/稳定顺序未入合同。 | 纳入 `JP07` | 纳入 `JP08` | 需澄清：运费支付归属/分摊未冻结。 |
...
| 按品类/卖家多指标 | 任意混合粒度组合 | 品类或卖家 | 无 | 排除。只有三项商品行指标有天然归属；混入订单或评价指标即产生归属歧义。 |
...
| D. 商品行天然维度 | `gmv`/`item_count`/`freight_amount` 按品类或卖家 | 480 | 70 | 70 | 30 |
...
| **总计** | 仅本矩阵的纳入范围 | **4,640** | **690** | **690** | **340** |
...
下一项可以且只能设计并审阅 `QuerySpec` schema 与 deterministic PostgreSQL Gold SQL renderer
的职责边界。开始前必须固定：
...
5. 在设计完成、代码与测试经审阅且用户确认前，不生成 QuerySpec、Gold SQL、训练行或启动 token 审计和 GPU 任务。
