...
| 现在处于哪一阶段？ | 已完成数据/环境/8-step QLoRA SFT/完整 Base-Adapter 评测和失败诊断，以及两条 26-step 受控训练与 adapter reload；正在等待各自完整的 base/adapter 质量评测。 |
...
| R2 SFT 质量门 | 先建立不回退的 SFT 基线 | 进行中 | 8-step adapter 的全量对照已是负向结果；26-step QLoRA/bf16 LoRA 训练和 reload 已完成，尚未完成质量对照。 |
