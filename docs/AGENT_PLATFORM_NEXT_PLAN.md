...
> 文档状态：实施路线；P0 会话/预算基础和嵌入宿主页历史交互已完成，P1 Text-to-SQL 已落地基础组件，运行链路和评测仍在完善。
> 更新日期：2026-08-03
...
- Catalog YAML、确定性检索器、问题路由和 working memory 已接入 Trusted Demo 的 SSE 主链路；SQL 修复契约、数据库错误脱敏和结果级校验已实现基础模块，但语义元数据尚未完整传入运行时，模型驱动修复的完整生命周期、置信度/拒答证据和在线模型语义准确率报告仍未完成。
