...
> 文档状态：实施路线；P0 会话/预算基础和嵌入宿主页历史交互已完成，P1 Text-to-SQL 第一阶段运行时合同已完成，修复生命周期和评测仍在完善。
> 更新日期：2026-08-03
...
- 60 条第一轮确定性用例和 3 条固定演示场景的安全与数据库 golden 校验；Text-to-SQL v2 专项回归当前 68 项通过；
- 6 条嵌入窗口浏览器回归（拖拽、缩放、移动端、图表尺寸、会话恢复）通过；starter UI 空会话不再写入历史。
...
- Catalog YAML、确定性检索器、问题路由、working memory 和服务器拥有的 `ResultContract` 已接入 Trusted Demo 的 SSE 主链路；结果合同已进入 Vanna `ToolContext`，固定 Prompt、Catalog trace、Agent Run 与 SQL 审计携带版本证据。模型驱动修复的完整生命周期、置信度/拒答证据、浏览器多轮回归和在线模型语义准确率报告仍未完成。
