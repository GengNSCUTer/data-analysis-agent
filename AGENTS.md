...
2026-09-03 补充：本轮 `QuerySpec`/renderer 已完成实现与测试，最终证据为专项 `43 passed`、相关运行时回归 `159 passed`。此前暂停点中的“下一项仅可实现”已完成；仍禁止物化训练数据、读取 protected holdout、执行 Gold SQL、启动 split/token 审计、GPU 训练或评测。下一步只能由用户先审阅 `olist_queryspec.py` 与 `test_olist_queryspec.py`，确认输入、输出、不变量、fail-closed 边界和测试空白后，才可单独设计受控物化与后续 Gold 准入。
