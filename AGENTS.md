...
2026-09-03 补充：本轮 `QuerySpec`/renderer 已完成实现与测试，并完成轻量加固；最终证据为专项 `45 passed`、相关运行时回归 `161 passed`。`create_validated()` 仅是显式构造入口，renderer 仍保留二次验证；时间序列强制共享 canonical time field，分组键表达式固定且 NULL 策略有测试。已新增受控批量物化器设计任务卡，但仍禁止物化训练数据、读取 protected holdout、执行 Gold SQL、启动 split/token 审计、GPU 训练或评测。下一步只能由用户先审阅物化器设计的输入、输出、family/split/holdout 审计与失败策略，确认后才可编写物化器。
