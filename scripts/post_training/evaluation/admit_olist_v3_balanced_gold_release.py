...
from scripts.post_training.data.build_olist_v3_balanced_release_candidates import (  # noqa: E402
    BUCKETS,
    RELEASE_VERSION as STRUCTURAL_RELEASE_VERSION,
    SPLITS,
    SPLIT_TARGETS,
)
...
def load_full_v3_gold_rows(directory: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    if (
        manifest.get("release_version") != STRUCTURAL_RELEASE_VERSION
        ...
        raise ValueError("structural materialization is not the expected passing v3 release")
    ...
    if (
        ...
        raise ValueError("v3 structural artifact hashes or row counts do not match its manifest")
    ...
    if len(query_by_seed) != len(query_rows) or set(query_by_seed) != set(gold_by_seed):
        raise ValueError("v3 QuerySpec/Gold seed identity does not form a one-to-one mapping")
    ...
    for seed_id in sorted(query_by_seed):
        ...
        if any(query_row.get(field) != gold_row.get(field) for field in identity_fields):
            raise ValueError(f"v3 QuerySpec/Gold structural identity drift for {seed_id}")
        ...
        if (
            artifact.get("query_spec_id") != spec.query_spec_id
            or artifact.get("sql_sha256") != sha256_text(artifact["sql"])
        ):
    ...
    if Counter((str(row["split"]), str(row["primary_bucket"])) for row in rows) != Counter(
        (split, bucket)
        for split in SPLITS
...
def semantic_review_seed_ids(rows: list[dict[str, Any]]) -> set[str]:
    ...
    for group in sorted(groups):
        ordered.append(
            min(groups[group], key=lambda row: sha256_text(f"{ADMISSION_VERSION}:{row['seed_id']}"))
        )
    ...
    selected.update(str(row["seed_id"]) for row in remaining[: SEMANTIC_REVIEW_SAMPLE_SIZE - len(selected)])
    return selected
...
async def admit_release(
    ...
    *,
    ...
    materialization_dir = _external_existing_dir(materialization_dir, "v3 structural materialization")
    ...
    async def review_record(record: dict[str, Any]) -> None:
        ...
        prompt = review_prompt(spec, str(record["gold_sql"]), str(record["validated_result_summary"]))
    ...
    counts = {
        ...
        "needs_human_review": sum(record["admission_status"] == "needs_human_review" for record in records),
    ...
    try:
        ...
        aggregate = {
            ...
            "source": {
                "structural_manifest_sha256": sha256_file(materialization_dir / "materialization_manifest.json"),
                "structural_query_specs_sha256": manifest["outputs"]["query_specs_jsonl"]["sha256"],
                "structural_gold_sql_sha256": manifest["outputs"]["gold_sql_jsonl"]["sha256"],
            ...
            "record_hashes": [
                {
                    ...
                    "validated_result_summary_sha256": record.get("validated_result_summary_sha256"),
