#!/usr/bin/env bash
# Run the frozen Olist v3.1 SQL-only LoRA two-epoch training after smoke pass.
set -Eeuo pipefail
...
RUN_DIR="${RUN_DIR:-/disk2/gengnan/data-analysis-agent-data/experiments/qwen25coder15b-olist-v3_1-sql-only-bf16-lora-full2epoch-v1-20260915}"
...
echo "[experiment] olist_v3_1_sql_only_bf16_lora_full2epoch_v1"
...
PYTHONPATH="${ROOT}" "${PYTHON}" -m scripts.post_training.training.run_post_training_sft_smoke \
  ...
  --experiment-label olist_v3_1_sql_only_bf16_lora_full2epoch_v1
