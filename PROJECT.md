...
- 2026-09-04 已完成 protected-family summary 与小批 Gold 准入闭环：受限人工审阅将当前 QuerySpec 可表达的 protected 数据查询映射为 17 个外部 family，summary/evidence 均只存仓库外。物化器新增 evidence 强制绑定，验证 summary hash、family 数、版本和 WorkspacePin；并将多指标 family identity 规范为无序集合，防止只改列顺序绕过碰撞。选取 6 条无碰撞 seed 外部物化后，全部通过 SqlPolicy、`daa_analytics_reader`、QuerySpec 派生 ResultValidator 合同和 DeepSeek-V4-Flash advisory-only 语义审阅（6 admitted、0 rejected、0 needs-human-review）。这不是训练行、训练集或模型质量结论；下一项仅为该批构造真实运行时 QueryPlan/Prompt 和人工审阅的中文业务 query 变体。
