...
- 已完成后训练准备第一阶段：`ChartContract` 与归因三态接入运行时，`QueryPlan`/`ResultContract`/审计记录携带归因需求证据；`docs/post-training-data-protocol.md` 固化样本字段、来源、脱敏、评测和训练边界，`evals/manifests/post_training_holdout_v1.yaml` 将 v2 60 条 golden 永久隔离。训练候选采集、schema-disjoint train/validation 切分、tokenizer/forward smoke 和极小 SFT 工程 smoke 均在独立外部实验目录完成；下一步是在冻结协议下做 base vs adapter 质量对比。
- 2026-08-26 官方 Spider release rebaseline 已启动：将后训练评测启动脚本的 `DATASET_VERSION`/`MODEL_ID` 从硬编码改为显式配置，避免新旧数据集混淆；官方包 2-case Base smoke 已通过（官方 `dev.json`/`tables.json` 哈希、Qwen 1.5B QLoRA 加载、逻辑 CUDA 1 -> 物理 GPU 3 UUID guard 均匹配）。完整官方 Base 与 26-step QLoRA Adapter 已在两张独立 RTX 4090 上并行生成：Base 使用逻辑 CUDA 1 -> 物理 GPU 3，Adapter 使用逻辑 CUDA 0 -> 物理 GPU 2；输出分别位于仓库外 `qwen25coder15b-base-spider-official-v1-20260826` 与 `qwen25coder15b-adapter-spider-official-v1-20260826`。两者均使用官方 dev、同一 prompt/decode 和 pinned evaluator 合同，不复用旧 Kaggle mirror prediction/score；生成完成后再分别运行 SQLite/Test Suite 并做 paired analysis。
