...
- 2026-08-26 官方 Spider release rebaseline 已启动：将后训练评测启动脚本的 `DATASET_VERSION`/`MODEL_ID` 从硬编码改为显式配置，避免新旧数据集混淆；官方包 2-case Base smoke 已通过（官方 `dev.json`/`tables.json` 哈希、Qwen 1.5B QLoRA 加载、逻辑 CUDA 1 -> 物理 GPU 3 UUID guard 均匹配）。完整官方 Base 生成、SQLite diagnostics 和 pinned Test Suite bridge 在独立 `screen` `daa-qwen15b-base-official-v1` 中运行，输出目录为仓库外 `qwen25coder15b-base-spider-official-v1-20260826`；完成前不启动 Adapter，不复用旧 Kaggle mirror 的 prediction 或 score。
...
| 日期 | 事项 | 结论 |
| --- | --- | --- |
| 2026-08-26 | 官方 Spider release Base 重新评测启动 | 评测脚本新增显式 `DATASET_VERSION` 和 `MODEL_ID`，保留历史 Kaggle 默认值以保证旧实验可复现。官方数据 2-case smoke 通过：`dev.json`/`tables.json` 哈希与外部 manifest 一致，Qwen 1.5B 4-bit NF4 在逻辑 CUDA `1`、物理 GPU `3`、UUID `GPU-10863af0-8588-7625-5609-640ba794f64b` 上通过守卫。完整 Base 任务已在 detached `screen` `daa-qwen15b-base-official-v1` 启动，独立运行目录为 `/disk2/gengnan/data-analysis-agent-data/experiments/qwen25coder15b-base-spider-official-v1-20260826`；生成完成后才运行 SQLite/Test Suite，暂不产生官方分数或启动 Adapter。 |
