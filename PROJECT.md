...
- 2026-09-02 十指标 Olist 领域 SFT coverage 已冻结：新增 `olist-domain-sft-coverage-matrix-v2.md`，保留四指标 v1 为历史快照。所有十项均可覆盖标量、确认绝对时窗、各自时间字段的序列和客户州；客户城市的关联粒度安全，但高基数输出的排序/截断尚未进入真实 Prompt，因此暂缓。只有 `gmv`、`item_count`、`freight_amount` 可天然按品类或卖家聚合。订单/评价事实在品类、卖家或支付方式下的归属/分摊仍 fail closed；跨购买/评价时间字段的时间序列、Top-N、自由筛选、相对时间、比较/因果和非 SQL 路由仍排除。物化前最多规划 `4,640/690/690` train/validation/in-domain test 行、340 个 family，不表示样本已构造或足以证明模型效果。下一项只能设计 QuerySpec 与 deterministic PostgreSQL Gold renderer 的职责边界。
...
| 2026-09-02 | Olist 领域 Candidate SQL SFT v2 覆盖矩阵冻结 | 基于 `olist-catalog-v2` / 十项业务指标合同，新增 `docs/post-training/data/olist-domain-sft-coverage-matrix-v2.md`，不改写四指标 v1 历史快照。十项均纳入标量、确认时窗、自身时间字段序列和客户州；客户城市因高基数输出的排序/截断合同缺失而暂缓。`gmv`、`item_count`、`freight_amount` 额外纳入按品类/卖家。订单和评价粒度在品类、卖家、支付方式下的归属/分摊问题仍需澄清或排除；多指标只纳入标量、州级、购买时间八指标序列或评价时间两指标序列，禁止混合时间字段的序列。规划上限为 `4,640/690/690` 行、至少 340 个 family，未生成任何 QuerySpec、SQL、JSONL 或训练任务。下一项只能设计 QuerySpec schema 与确定性 PostgreSQL Gold renderer 的职责边界。 |
